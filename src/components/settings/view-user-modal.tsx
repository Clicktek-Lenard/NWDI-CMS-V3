...
function parsePermissions(role: string | null) {
  if (!role) return [];
  try {
    const arr = JSON.parse(role);
    if (!Array.isArray(arr)) return [];
    return arr.map((r: { module: string; tab: string }) => permKey(r.module, r.tab));
  } catch {
    return [];
  }
}
...
export function ViewUserModal({ open, user, onClose, onEdit }: Props) {
  ...
  return (
    ...
    >
      <div className="relative w-full max-w-lg rounded-2xl border border-slate-200 bg-white shadow-2xl max-h-[90vh] flex flex-col">
        ...
        <div className="overflow-y-auto px-6 py-5">
          ...
          <div className="mb-6 flex items-center gap-4">
            ...
            <div>
              ...
              {user.ldap_import === 1 && (
                <span className="ml-1.5 mt-1 inline-flex items-center rounded-full bg-purple-100 px-2.5 py-0.5 text-xs font-medium text-purple-700">
                  LDAP
                </span>
              )}
            </div>
          ...
          <div className="mt-5">
            <div className="mb-3 flex items-center gap-2">
              ...
              <span className="text-sm font-medium text-slate-700">Module Access</span>
              ...
              <div className="space-y-3">
                {PERMISSION_GROUPS.map((group) => {
                  const granted = group.permissions.filter((p) =>
                    permissions.includes(permKey(p.module, p.tab))
                  );
