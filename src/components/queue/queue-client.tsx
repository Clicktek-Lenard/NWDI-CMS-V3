...
export function QueueClient() {
  ...
  const handleAssignAccession = useCallback(async () => {
    ...
    try {
      ...
      setAssignMsg({ type: "err", text: e instanceof Error ? e.message : "Failed to assign accession numbers. Try again." });
    } finally {
      setAssigning(false);
    }
  }, [fetchQueue]);
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center py-32">
        ...
        <span className="ml-3 text-sm text-slate-500 dark:text-slate-400">Loading today&apos;s queue...</span>
      </div>
    );
  }
  ...
  return (
    <div>
      ...
      <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-2xl font-bold text-slate-800 dark:text-slate-100">Queue Today</h1>
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="inline-flex items-center gap-2 rounded-xl border border-amber-300 bg-amber-50 px-4 py-2 text-sm font-semibold text-amber-800 shadow-sm transition-all hover:bg-amber-100 active:scale-[0.98] disabled:opacity-60 disabled:cursor-not-allowed dark:border-amber-700 dark:bg-amber-900/20 dark:text-amber-300 dark:hover:bg-amber-900/40"
          >
            {assigning ? (
      ...
      <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="relative">
          ...
          <input
            ...
            className="w-full rounded-lg border border-slate-200 bg-white py-2 pl-9 pr-4 text-sm text-slate-700 placeholder:text-slate-400 focus:border-blue-400 focus:outline-none focus:ring-2 focus:ring-blue-400/20 sm:w-72 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-200 dark:placeholder:text-slate-500 dark:focus:border-blue-500"
          />
        </div>
      </div>
          ...
          <table className="w-full text-left text-sm">
            <thead>
              <tr className="border-b border-slate-100 bg-slate-50/80 dark:border-slate-700 dark:bg-slate-800">
                ...
                <th className="whitespace-nowrap px-4 py-3 font-semibold text-slate-600 dark:text-slate-300">Accession No</th>
                <th className="whitespace-nowrap px-4 py-3 font-semibold text-slate-600 dark:text-slate-300">Patient Type</th>
                ...
                <th className="whitespace-nowrap px-4 py-3 font-semibold text-slate-600 dark:text-slate-300">Added By</th>
                <th className="whitespace-nowrap px-4 py-3 font-semibold text-slate-600 dark:text-slate-300"></th>
              </tr>
            </thead>
                ...
                filtered.map((q) => {
                  ...
                  return (
                    <tr key={q.id} className="transition-colors hover:bg-slate-50/60 dark:hover:bg-slate-800/60">
                      ...
                      <td className="whitespace-nowrap px-4 py-3">
                        <Link href={`/queue/${q.id}/edit`} className="font-mono text-xs text-blue-600 hover:underline dark:text-blue-400">
                          {q.code}
                        </Link>
                      </td>
                      ...
                      <td className="whitespace-nowrap px-4 py-3 font-mono text-xs text-slate-600 dark:text-slate-400">
                        {q.accessionNo || <span className="text-slate-300 dark:text-slate-600">—</span>}
                      </td>
                      <td className="whitespace-nowrap px-4 py-3 text-xs text-slate-600 dark:text-slate-400">
                        {q.patientType || <span className="text-slate-300 dark:text-slate-600">—</span>}
                      </td>
                      <td className="whitespace-nowrap px-4 py-3">
                        <span
                      ...
                      <td className="whitespace-nowrap px-4 py-3">
                        <span className="text-xs text-slate-700 dark:text-slate-300">{formatTime(q.queueDateTime)}</span>
                        <span className="ml-2 text-[11px] text-slate-400 dark:text-slate-500">{timeAgo(q.queueDateTime)}</span>
                      </td>
                      ...
                      <td className="whitespace-nowrap px-4 py-3">
                        {q.statusCode === 210 && (
                          <button
                            ...
                            className="inline-flex items-center gap-1 rounded-lg border border-teal-300 bg-teal-50 px-2.5 py-1 text-xs font-semibold text-teal-700 transition-all hover:bg-teal-100 disabled:opacity-50 dark:border-teal-700 dark:bg-teal-900/20 dark:text-teal-300 dark:hover:bg-teal-900/40"
                          >
...
function StatCard({
  ...
  return (
    <div className={`rounded-xl border border-slate-200 p-4 dark:border-slate-700 ${p.bg}`}>
      <div className="flex items-center justify-between">
        <p className="truncate pr-2 text-sm font-medium text-slate-600 dark:text-slate-300">{label}</p>
        <div className={`flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-lg bg-white/60 dark:bg-black/20 ${p.text}`}>
          {icon}
        </div>
      </div>
